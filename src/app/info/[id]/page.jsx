...
export function generateStaticParams() {
  ...
  // service_info: home-service_info-1, home-service_info-2 ...
  services2.forEach((_, i) => {
    params.push({ id: `home-service_info-${i + 1}` });
  });

  // blog_info by index: blog-blog_info-1, blog-blog_info-2 ...
  blogs2.forEach((_, i) => {
    params.push({ id: `blog-blog_info-${i + 1}` });
  });

  // blog_info by slug (new pattern)
  blogs2.forEach((blog) => {
  ...
  brands2.forEach((brand) => {
    const brandProjects = projects2.filter(
      (p) => p.forBrand === brand.pathname
    );
    brandProjects.forEach((_, i) => {
      params.push({ id: `brand-project_info-${i + 1}-${brand.pathname}` });
    });
  });
